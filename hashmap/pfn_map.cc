...
absl::flat_hash_map<int, int> pfn_map_cnt;
absl::flat_hash_map<int, int> pfn_map;

query_arg_t q;


int putAndIncrement(int key) {
	int value = -1;
	const auto &it = pfn_map_cnt.find(key);
	if (it != pfn_map_cnt.end()) {
		value = it->second++;
		// std::cout << it->second << "\n";
	} else {
    pfn_map_cnt.insert({key, 1});
  }
	return value;
}
...
int main(int argc, char **argv)
{
    ...
    snprintf(pagemap_file, sizeof(pagemap_file), "/proc/%ju/pagemap", (uintmax_t)pid);
    snprintf(query_file_name, sizeof(pagemap_file), "/dev/query");
    ...
    // char *query_file_name = "/dev/query";
    int query_fd;
  	query_fd = open(query_file_name, O_RDWR);
    if (query_fd < 0) {
        perror("open query");
        return EXIT_FAILURE;
    }
                ...
                /* Get info about all pages in this page range with pagemap. */
                {
                    ...
                    for (uintptr_t addr = low; addr < high; addr += sysconf(_SC_PAGE_SIZE)) {
                        ...
                        if (!pagemap_get_entry(&entry, pagemap_fd, addr)) {
                          ...
                          // int pfn_now = 1;
                          if(pfn_now != 0 ) {
                            int prevCnt = putAndIncrement(pfn_now);
                            if( prevCnt == -1 ) {
                              pfn_map.insert({pfn_now, (uintmax_t)q.vfn}); // insert pfn -> vfn
                            } else {                                
                              // printf("%jx %jx %jx %d\n",
                              //   (uintmax_t)addr,
                              //   (uintmax_t)entry.pfn,
    ...
    // for (auto const& pair: pfn_map_cnt) {
